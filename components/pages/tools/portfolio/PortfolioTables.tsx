      ...
      >
        <div className="flex items-center gap-4">
          <div className="h-12 w-12 rounded-full flex items-center justify-center flex-shrink-0">
            <Image
              src={
                asset.logoUrl || getTokenLogoUrlWithFallbackSync(asset.asset_type, asset.metadata)
              }
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between mb-1">
              ...
              <div className={`font-bold text-base ${GeistMono.className}`}>
                {asset.price && asset.price > 0 ? formatCurrency(asset.value || 0) : "—"}
              </div>
            </div>

            <div className="flex items-center justify-between text-sm text-muted-foreground">
              <span className="truncate">{asset.metadata?.name || "Unknown Asset"}</span>
              <div className="flex items-center gap-3 flex-shrink-0">
                ...
                {asset.price && asset.price > 0 && (
                  <>
                    <span>•</span>
                    <span className={GeistMono.className}>{formatCurrency(asset.price)}</span>
                  </>
                )}
              </div>
            </div>
    ...
    <React.Fragment key={`asset-${asset.asset_type}-${asset.amount}`}>
      <TableRow
        className={cn(
          ...
          selectedItem?.asset_type === asset.asset_type && "bg-muted/40 rounded-lg"
        )}
        onClick={() => onItemSelect(asset)}
      >
        <TableCell className="py-2 w-1/2 sm:w-2/5">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                <span className="font-medium text-sm">{asset.metadata?.symbol || "Unknown"}</span>
                {!asset.isVerified && (
                  <span
                    className="text-xs text-amber-500 dark:text-amber-400"
                    title="Unverified token"
        ...
        <TableCell className="py-2 w-1/4 sm:w-1/5 text-right pr-4">
          <div className={`font-medium text-sm text-right ${GeistMono.className}`}>
            {asset.price && asset.price > 0 ? formatCurrency(asset.value || 0) : "—"}
          </div>
        </TableCell>
      </TableRow>
      ...
      {safeVisibleAssets.length > 0 ? (
        <>
          ...
          {isMobile ? (
            <div className="space-y-2">
              {aptAsset && (
                <AssetCard asset={aptAsset} showDivider={limitedOtherAssets.length > 0} />
              )}
        ...
        <div className="text-center py-8 text-muted-foreground">
          ...
          <p>{showOnlyVerified ? "No verified assets found" : "No assets found"}</p>
  ...
  if (!groupedDeFiPositions || groupedDeFiPositions.length === 0) {
    return (
      <div className="text-center py-8 text-muted-foreground">
        ...
        <p className="text-sm">Start using DeFi protocols on Aptos to see your positions here</p>
      </div>
    );
  }
