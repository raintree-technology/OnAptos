...
export const UnifiedNFTGrid: React.FC<UnifiedNFTGridProps> = ({
  ...
  const renderNFTCard = (nft: NFT, index: number) => {
    ...
    const sanitizedNFT = sanitizeNFTMetadata(nft);
    const imageUrl = sanitizeImageUrl(nft.cdn_image_uri || nft.token_uri, "/placeholder.jpg");
      ...
      >
        ...
        {showMetadata && (
          <div className="mt-2 space-y-1">
            <p className="text-sm font-medium truncate" title={sanitizedNFT.token_name}>
  ...
  if (!nftsLoading && filteredNFTs.length === 0) {
    return (
      <div className={cn("flex items-center justify-center min-h-[400px]", containerClassName)}>
              ...
              <CollapsibleTrigger asChild>
                ...
                >
                  <div className="flex items-center gap-3">
                    <span className="font-medium truncate max-w-[200px]" title={collection}>
                      {collection}
                    </span>
