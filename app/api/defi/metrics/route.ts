...
export async function GET() {
  try {
    const metrics = await serverCached(
      ["defi:metrics"],
      async () => {
      ...
      { revalidate: CACHE_DURATIONS.MEDIUM, tags: ["defi:metrics"] }
    );

    apiLogger.info("DeFi metrics fetched successfully", { tvl: metrics.tvl });

    return NextResponse.json(metrics, { headers: getCacheHeaders(CACHE_DURATIONS.MEDIUM) });
  } catch (error) {
