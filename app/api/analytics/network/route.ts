...
function calculateNetworkMetrics(networkData: any) {
  ...
  const recent24hTxs = userTxs.filter((tx: any) => new Date(tx.timestamp).getTime() > oneDayAgo);

  const recent7dTxs = userTxs.filter((tx: any) => new Date(tx.timestamp).getTime() > oneWeekAgo);
  ...
  // Calculate averages
  const avgBlockTime =
    blockTimes.length > 0 ? blockTimes.reduce((sum, time) => sum + time, 0) / blockTimes.length : 0;

  const medianBlockTime =
    blockTimes.length > 0 ? blockTimes.sort((a, b) => a - b)[Math.floor(blockTimes.length / 2)] : 0;
  ...
  return {
    ...
    currentTPS: avgBlockTime > 0 ? parseFloat((1 / avgBlockTime).toFixed(2)) : 0,
    ...
    uniqueAccountTransactions: new Set(accountTxs.map((tx: any) => tx.account_address)).size,

    // Timing analysis
    blockTimeStats: {
      min: Math.min(...blockTimes),
      max: Math.max(...blockTimes),
      p95: blockTimes.sort((a, b) => a - b)[Math.floor(blockTimes.length * 0.95)] || 0,
...
function calculateAssetMetrics(assetData: any) {
  ...
  const recent24hActivities = activities.filter(
    (activity: any) => new Date(activity.transaction_timestamp).getTime() > oneDayAgo
  );

  const activeAssets24h = new Set(recent24hActivities.map((a: any) => a.asset_type));
  const activeUsers24h = new Set(recent24hActivities.map((a: any) => a.owner_address));

  // Asset distribution analysis
  const assetHolderCounts = balances.reduce((acc: Record<string, number>, balance: any) => {
  ...
  return {
    ...
    averageHoldersPerAsset:
      uniqueAssets.size > 0 ? parseFloat((uniqueHolders.size / uniqueAssets.size).toFixed(2)) : 0,
...
function calculateNFTMetrics(nftData: any) {
  ...
  return {
    ...
    averageSupplyPerCollection:
      totalCollections > 0 ? parseFloat((totalSupply / totalCollections).toFixed(2)) : 0,
