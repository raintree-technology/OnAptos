...
function calculateCostMultiplier(
  ...
  if (cost === "$2-4") {
    ...
    numericCost = cleanCost.includes("-")
      ? parseFloat(cleanCost.split("-")[1])
      : parseFloat(cleanCost);
  ...
  return { multiplier: multiplierText, severity };
}
...
export default function USDTComparisonPage() {
  ...
  const costCards = usdtCosts.map((item) => {
