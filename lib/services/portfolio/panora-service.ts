...
export class PanoraService {
   ...
   */
  static async getTokenPrices(tokenAddresses: string[]): Promise<PanoraPriceResponse[]> {
    ...
    const response = await cacheFirst({
      ...
      startTime: Date.now(),
      fetchFn: async () => {
        logger.info(`Fetching prices for specific tokens: ${tokenAddresses.join(", ")}`);
        ...
        logger.info(`Retrieved ${result.length} specific token prices from Panora API`);
        return result;
      },
    });
  ...
  static async getAssetPrice(assetType: string, symbol: string): Promise<TokenPriceData> {
    try {
      ...
      const panoraMatch = panoraPrices.find(
        (price) => price.faAddress === assetType || price.tokenAddress === assetType
      );
      ...
      logger.info(`Asset ${assetType} not found in Panora, no fallback available for ${symbol}`);
  ...
  static getSymbolForAssetType(assetType: string): string {
    ...
    if (assetType === COMMON_TOKENS.APT) {
      return "APT";
    }
